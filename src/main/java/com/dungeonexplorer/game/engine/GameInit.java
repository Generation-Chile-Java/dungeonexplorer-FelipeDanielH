...
import static main.java.com.dungeonexplorer.game.DungeonExplorer.CONTINUAR;

public class GameInit {

    public static void init(String nombre) throws FileNotFoundException {
        GameUI consola = new GameUI();
        DungeonEngine game = new DungeonEngine();
        Set<Integer> opcionesAccionesValidas;


        game.getPersonaje().setNombre(nombre);

        int accion = 999;
        // SE REPITE CADA VEZ QUE EL USUARIO NO ESCOGE UNA OPCION VALIDA DENTRO DEL MENU
        do {

            if(game.getPersonaje().getVidaActual()<=0){
                consola.showMessageContinuar();
                int opcionContinuar = consola.getUserInput("Elige una opcion:");
